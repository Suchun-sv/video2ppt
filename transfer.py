...
def main():
    ...
    config = {x[0]:x[1] for x in configFile.items("config")}

    #命令行
    parser = argparse.ArgumentParser()
    parser.add_argument("--videoPath","-v",help="videoPath")
    parser.add_argument('--time_interval','-t',type=int,help='time_interval')
    parser.add_argument('--pictureFolder','-p',help='pictureFoder')
    parser.add_argument('--reducePictureFolder','-r',help='reducePictureFolder')
    parser.add_argument('--pptName','-m', help='pptName')
    parser.add_argument('--threshold','-th', help='threshold')
    parser.add_argument('--debug', help="debug", action="store_true")
    parser.add_argument('--simple',"-s",help="simple mode,run in the videopath",action="store_true")

    args = parser.parse_args()
    if args.videoPath:
        if os.path.exists(args.videoPath):
            config['videopath'] = args.videoPath
        else:
            raise FileNotFoundError("videoPath not exists")
    if args.time_interval:
        config['time_interval'] = args.time_interval
        
    if args.pictureFolder:
        config['picturefolder'] = args.pictureFolder
        
    if args.reducePictureFolder:
        config['picturefolder_reduce'] = args.reducePictureFolder

    if args.pptName:
        config['pptname'] = args.pptName
    
    if args.threshold:
        config['threshold'] = args.threshold
    
    config['picturefolder'] = os.path.realpath(config['picturefolder'])
    config['picturefolder_reduce'] = os.path.realpath(config['picturefolder_reduce'])
    config['ppttemplate'] = os.path.realpath(config['ppttemplate'])
    config['pptname'] = os.path.realpath(config['pptname'])
    # 简洁模式
    if args.simple:
        config['videopath'] = os.path.realpath(config['videopath'])
        (filepath,tempfilename) = os.path.split(config['videopath'])
        (filename,extension) = os.path.splitext(tempfilename)
        config['picturefolder'] = os.path.join(filepath,'picturefolder')
        config['picturefolder_reduce'] = config['picturefolder']+'_reduce'

        config['pptname'] = os.path.join(filepath,filename+'.pptx')
    
    v2p = video2pptx(
                pictureFolder = config['picturefolder'],
                videoPath = os.path.realpath(config['videopath']),
                time_interval = int(config['time_interval']),
                )
    v2p.capFrame()
    similar_score = v2p.calcSimilar(config['picturefolder'])
    v2p.copyPictureBySimilar(float(config['threshold']),config['picturefolder'],reducePictureFolder = config['picturefolder_reduce'],similar_score=similar_score)
    v2p.createPPtx(config['pptname'],pptxTemplate=config['ppttemplate'],pictureFile=config['picturefolder_reduce'])
    
    if args.simple:
        shutil.rmtree(config['picturefolder'])
        shutil.rmtree(config['picturefolder_reduce'])
        #pass
